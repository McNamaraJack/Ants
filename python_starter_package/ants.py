...
class Ants():
    ...
    def distance(self, loc1, loc2):
        ...
        #def add_to_samples(loc):
        #    if loc in [x for v in samples.values() for x in v]:
        #        return False
        #    else:
        #        if self.passable(loc):
        #            samples[d+1].append(loc)
        #            return True
        #        return False

        #while loc2 not in samples[d]:
        #    samples[d+1] = []
        #    for a,b in samples[d]:
        #        if add_to_samples(((a + 1) % self.rows, b)):
        #            pass
        #        if add_to_samples(((a - 1) % self.rows, b)):
        #            pass
        #        if add_to_samples((a, (b + 1) % self.cols)):
        #            pass
        #        if add_to_samples((a, (b - 1) % self.cols)):
        #            pass
        #    d += 1
...
#
 ...
 #               break
#
  #          if self.time_remaining() < 1000:
        return heuristic(loc2,loc1)
#
